...
app.get('/player_info/:id', function(req, res) {
	var query = 'select id, name from football_players;'

  var id = req.params.id
  if(id == 'post'){
    console.log(req.query.player_choice)
    id = req.query.player_choice
  }
  var select_player = 'select * from football_players where id='+id+';'
  var game_count = 'select count(*) from football_games where '+id+'=ANY(players);'
  db.task('get-everything', task => {
    return task.batch([
      task.any(query),
      task.any(select_player),
      task.any(game_count)
    ])
  })
  .then(data => {
    selected_player = data[1][0]
    selected_player['games_played'] = parseInt(data[2][0].count)
